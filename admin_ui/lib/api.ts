 ...
 *
 * On the server (server components, route handlers) it reads the platform-
 * admin JWT from the httpOnly `helalien_admin_token` cookie automatically.
 * Client components must not call this directly — they go through this
 * app's own `/api/*` route handlers, which forward the cookie.
 */
