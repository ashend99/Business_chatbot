...
models/       SQLAlchemy ORM, one file per domain: tenants, documents,
              catalog, conversations, leads, orders, admin
...
services/     business logic: bot_engine, bot_tools, chunking, embeddings,
              publishing, notifications, onboarding, email
api/          FastAPI routers: auth/, superadmin/, tenant/ (one file per
              dashboard resource), bot/ (POST /bot/message)
...
- **`confirm_order`** — moves a draft order to `placed`. This is as far as
  the agent's authority goes — **payment is never handled by the agent**,
  always a human/off-platform step afterward. Two things must be true
  server-side before this succeeds (not just prompted — see below):
  contact info collected, and a fulfillment type (delivery/pickup/etc.) set.
...
1. **Pricing guardrail** (`_apply_pricing_guardrail`) — after generation,
   any `$`-amount in the reply must appear in this turn's trusted tool
   output (`search_catalog`/`browse_catalog`/`update_order`/`confirm_order`
   combined); otherwise the reply is discarded for the tool's own text
   (with internal `[variant_id: ...]` tags stripped — this leaked into a
   customer-facing reply once before that fix).
2. **Order-confirmation guardrail** (`_apply_order_confirmation_guardrail`)
   ...
   while the DB still showed `status=draft`.
3. **Server-side enforcement in `repos/orders.py`'s `confirm_order`** —
   raises `MissingRequiredContactInfo` / `MissingFulfillmentInfo` instead
   of trusting the agent to have asked first. `MissingRequiredContactInfo`
...
**Settings is still a placeholder** — never built past the nav stub.
...
- **`eval/scenarios.yaml`** — a persona (instructions for a simulated
  customer) + `success_criteria` (graded later) per scenario.
...
- **Settings page** — nav entry and route exist, content is a placeholder.
- **Currency display** — prices are always shown with a hardcoded `$`
  prefix (`bot_tools.py`, dashboard `formatMoney`), even though a tenant's
  real prices/documents may be in a different currency (the seeded dev
  tenant's actual prices are LKR). Noted, not fixed.
